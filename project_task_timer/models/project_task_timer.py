...
class ProjectTaskTimeSheet(models.Model):
    ...
    reason_why=fields.Char(string='reason why not finished')
...
class ProjectTaskTimer(models.Model):
    ...
    task_done = fields.Boolean('Finish ?')
    select_reason=fields.Selection([ ('Electricity', 'Electricity problem '),('Personal', ' personal problems '),], default="Personal", string="not finished due to" )
    ...
    def toggle_start(self):
        if self.task_timer is True:
            ...
            for time_line in time_line_obj.search(domain):
                time_line.write({'date_end': fields.Datetime.now() , 'reason_why':self.select_reason})
